...
Valu And::eval(const Defs& defs, const Ctxt& ctxt) const {
    ...
    if (std::holds_alternative<bool>(lv)
        && std::holds_alternative<bool>(rv)) {
        int ln = std::get<bool>(lv);
        int rn = std::get<bool>(rv);
        return Valu {ln&&rn};
    } else {
...
Valu Oorr::eval(const Defs& defs, const Ctxt& ctxt) const {
    Valu lv = left->eval(defs,ctxt);
    Valu rv = rght->eval(defs,ctxt);
    if (std::holds_alternative<bool>(lv)
        && std::holds_alternative<bool>(rv)) {
...
Valu IntC::eval([[maybe_unused]] const Defs& defs, const Ctxt& ctxt) const {
    //
    // The integer conversion operation does hing in this
    // version of DWISLPY.
    //
    Valu v = expn->eval(defs,ctxt);
    if (std::holds_alternative<int>(v)) {
...
void Oorr::output(std::ostream& os) const {
  os << "(";
  left->output(os);
  os << " or ";
  rght->output(os);
...
void Oorr::dump(int level) const {
    dump_indent(level);
    std::cout << "OORR" << std::endl;
    left->dump(level+1);
    rght->dump(level+1);
}
